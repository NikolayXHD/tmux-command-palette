  ...
  tmux-command-palette.toml   — personal aliases, kept next to your tmux.conf
    (falls back to the plugin dir); auto-maintained on every palette run:
      * bindings that differ from every expectation (stock defaults and
        palette.toml commands) are appended with a starting alias from the
        terminology glossary (jargon in the command -> everyday words, e.g.
        window -> tab; never a claim about the full action);
      * when the command of a known binding changes, alias is reset to ""
        (so a stale alias cannot lie) and the old value is kept in alias_was;
...
def validate_file(path, label: str, keyed_by_command: bool) -> str | None:
    ...
    for entry in entries:
        ...
        if not keyed_by_command and not entry.get("alias", "") and not entry.get("alias_was", ""):
...
def cmd_check() -> int:
    ...
    a command that matches some live binding.
    """
    exit_code = 0
...
def main() -> int:
    ...
    parser.add_argument("--gen", action="store_true", help="sync the personal aliases file")
    parser.add_argument("--check", action="store_true", help="validate alias files, warn about uncovered stock bindings")
    parser.add_argument("--exec", metavar="IDENT", help="execute entry table|key if it can run detached")
    parser.add_argument("--client-tty", metavar="TTY", default="", help="tty of the client that opened the palette")
