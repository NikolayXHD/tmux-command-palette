...
def _has_option_before_quote(command: str, option: str) -> bool:
    """Whether `option` appears as a token outside quoted text and {} blocks.

    tmux config syntax may quote strings and wrap menu items in {}; an
    option mentioned there is not a real option of this command.
    """
    quote = ""
    depth = 0
    i = 0
    ...
    while i < n:
        ...
        if quote:
            ...
            depth -= 1
        elif depth == 0 and (i == 0 or command[i - 1].isspace()):
            end = i
            while end < n and not command[end].isspace() and command[end] not in "\"'{":
                end += 1
            if command[i:end] == option:
                return True
            i = end
            continue
        i += 1
...
def cmd_exec(selection: str, client_tty: str = "") -> int:
    ...
    if entry is None or not can_exec_command(
        entry.command, have_client_tty=bool(client_tty)
    ):
        return 0
    ...
    if head.startswith(INTERACTIVE_PREFIXES) or head.startswith("run-shell"):
        ...
        if proc.stdin is None:
            ...
            return 1
        try:
            proc.stdin.write(source_file_command(command) + "\n")
            proc.stdin.close()
    ...
    try:
        result = subprocess.run(
            ["tmux", "source-file", "-"],
            input=source_file_command(command) + "\n",
            capture_output=True,
            text=True,
        )
    except OSError as error:
